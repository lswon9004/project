...
public interface ApprovalDao {
	...
	@Select("select * from approval where approver1_empno = #{empno} or approver2_empno = #{empno} order by created_date desc limit #{start} , 10")
	List<ApprovalDto> approvalStatus(@Param("empno")int empno,@Param("start")int start);
	@Select("select count(*) from approval where approver1_empno = #{empno} or approver2_empno = #{empno}")
