        ...
        <main>
			...
			<col style="width:12%;" />
				...
				<c:if test="${begin > pageNum }">
					<a href="/boards/search?p=${begin-1 }&date=${date}&author=${empno}&title=${title}" class="page prv">&lt;</a>
				</c:if>
				<c:forEach begin="${begin }" end="${end}" var="i">
					<a href="/boards/search?p=${i}&date=${date}&author=${empno}&title=${title}">${i}</a>
				</c:forEach>
				<c:if test="${end < totalPages }">
					<a href="/boards/search?p=${end+1}&date=${date}&author=${empno}&title=${title}" class="page next">&gt;</a>
				</c:if>
			</div>
				</div>
            </section>
